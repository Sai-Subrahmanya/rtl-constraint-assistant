...
def _validation(
    *, coverage: CoverageReport | None = None, status: str = "PASS"
) -> ValidationResult:
...
def _review(cset: ConstraintSet, *, warnings: bool = False):
    ...
    if warnings:
        return approve_review(
            review, cset, actor=ReviewActor.from_value("reviewer"), comment="approved"
        )
    return approve_review(
        review, cset, actor=ReviewActor.from_value("reviewer"), comment="approved"
    )
...
def _clean_inputs(cset: ConstraintSet):
    ...
    policy = ReleasePolicy(
        require_configuration=False, require_design=False, require_timing_graph=False
    )
    return {
        "policy": policy,
        "review": review,
...
def _released(cset: ConstraintSet, **overrides):
    ...
    args = _clean_inputs(cset)
    args.update(overrides)
    return release_constraint_set(
        candidate, cset, actor=ReviewActor.from_value("releaser"), comment="release", **args
    )
...
def test_release_is_explicit_immutable_successor_and_not_external_signoff():
    cset = _ucm()
    candidate = _candidate(cset)
    released = release_constraint_set(
        candidate, cset, actor=ReviewActor.from_value("r"), comment="explicit"
    )
...
def test_missing_review_fails_closed():
    ...
    assert any(
        item.category == "REVIEW" and item.severity == ReleaseIssueSeverity.BLOCKER
...
def test_review_bound_to_other_ucm_fails_closed():
    first = _ucm()
    second = _ucm()
    second.create_clock("other", 20e-9, source="other", source_kind=SourceKind.USER)
    args = _clean_inputs(second)
    args["review"] = _review(first)
...
def test_nonapproved_review_is_not_releaseable():
    cset = _ucm()
    args = _clean_inputs(cset)
    args["review"] = create_constraint_review(
        cset, policy=ReviewPolicy(), readiness=args["readiness"], validation=args["validation"]
    )
    candidate = create_release_candidate(cset, **args)
    assert candidate.status == ReleaseStatus.BLOCKED
...
def test_validation_warning_requires_explicit_warning_policy():
    cset = _ucm()
    args = _clean_inputs(cset)
    args["validation"] = _validation(status="PASS_WITH_WARNINGS")
    candidate = create_release_candidate(cset, **args)
    ...
    released = release_constraint_set(
        candidate, cset, actor=ReviewActor.from_value("r"), **(args | {"policy": allowed})
    )
...
def test_coverage_unknown_or_incomplete_fails_closed():
    ...
    candidate = create_release_candidate(cset, **args)
    assert any(
        item.category == "COVERAGE" and item.severity == ReleaseIssueSeverity.BLOCKER
        for item in candidate.issues
    )
...
def test_unknown_or_conflicting_scope_is_invalid(selected, all_active):
    ...
    candidate = create_release_candidate(
        ...
        all_active_scenarios=all_active,
        **_clean_inputs(cset),
    )
    assert candidate.status == ReleaseStatus.INVALID
    assert any(
        item.category == "MCMM_SCOPE" and item.severity == ReleaseIssueSeverity.BLOCKER
        for item in candidate.issues
    )
...
def test_policy_can_require_all_active_scenarios():
    ...
    candidate = create_release_candidate(
        cset, scenario_ids=("FAST",), **(args | {"policy": policy})
    )
    assert candidate.status == ReleaseStatus.READY
...
def test_required_supplied_artifact_hash_is_bound_without_source_path(tmp_path: Path):
    ...
    args = _clean_inputs(cset)
    args["policy"] = replace(args["policy"], required_artifact_kinds=("trace",))
    candidate = create_release_candidate(cset, additional_artifacts={"trace": artifact}, **args)
    assert candidate.status == ReleaseStatus.READY
    artifact_evidence = next(
        item for item in candidate.evidence if item.authority == "ARTIFACT:trace"
    )
...
def test_revoke_creates_immutable_successor_and_can_not_be_released_again():
    ...
    revoked = revoke_release(
        released,
        cset,
        actor=ReviewActor.from_value("releaser"),
        comment="withdraw",
        recorded_at=_EPOCH,
    )
...
def test_package_is_explicit_reproducible_and_statelessly_verified(tmp_path: Path):
    ...
    args = _clean_inputs(cset)
    released = release_constraint_set(
        create_release_candidate(cset, **args),
        cset,
        actor=ReviewActor.from_value("r"),
        comment="go",
        **args,
    )
...
def test_package_rejects_candidate_stale_or_revoked_records(tmp_path: Path):
    ...
    released = _released(cset)
    revoked = revoke_release(released, cset)
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"require_sdc": True},
        {"allow_review_with_warnings": True},
...
def test_default_policy_requires_exact_configuration_design_and_timing_identities():
    ...
    blockers = {
        item.category for item in candidate.issues if item.severity == ReleaseIssueSeverity.BLOCKER
    }
...
def test_review_scope_must_cover_the_exact_release_scope():
    ...
    review = approve_review(
        review, cset, actor=ReviewActor.from_value("reviewer"), comment="FAST only"
    )
    candidate = create_release_candidate(cset, **(args | {"review": review}))
    assert candidate.status == ReleaseStatus.BLOCKED
    assert any(
        item.category == "REVIEW" and "scope" in item.message.lower() for item in candidate.issues
    )
