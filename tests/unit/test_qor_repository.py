...
def _qor(
    *,
    ...
    scenario: str = "SLOW",
    setup_ns: float | None = 1.0,
    hold_ns: float | None = 0.2,
    area: float | None = 100.0,
    area_proxy: float | None = None,
    ...
    return QoRResult(
        ...
        hold_tns=0.0 if hold_ns is not None else None,
        hold_violations=0,
        whs=hold_ns * 1e-9 if hold_ns is not None else None,
        ths=0.0,
        ...
        or (PowerStatus.AVAILABLE.value if power is not None else PowerStatus.UNAVAILABLE.value),
        constraint_quality=0.9,
        validation_errors=0,
        unsafe_exceptions=0,
        margin_headroom_ns=0.8,
        margin_utilization=0.2,
        congestion=0.1,
        runtime_seconds=3.0,
        critical_setup=CriticalPath(
            startpoint="a", endpoint="b", slack=setup_ns * 1e-9 if setup_ns is not None else None
        ),
        wns_percentiles={"p50": 0.5e-9},
        timing_distribution={"met": 4},
...
def _manifest(
    tmp_path: Path, run_id: str, *, candidate_id: str = "C000"
    ...
    manifest = RunManifest(
        ...
        artifact_hashes={
            "qor": hash_file(run_dir / "qor.json"),
            "sdc": hash_file(run_dir / "generated.sdc"),
        },
        tool_identity={"opensta": {"version": "2.5"}},
        input_hashes={
            "rtl": {"rtl/top.v": "rtl-hash"},
            "includes": {"inc": "inc-hash"},
            "libs": {"cells.lib": "lib-hash"},
        },
...
def test_01_database_creation_schema_version_and_application_identifier(tmp_path):
    ...
    assert (
        conn.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0] == SCHEMA_VERSION
    )
...
def test_02_new_database_migrates_and_migration_failure_rolls_back(tmp_path, monkeypatch):
    ...
    assert "artifact_fingerprint" in {
        row[1] for row in conn.execute("PRAGMA table_info(evaluations)")
    }
...
def test_04_run_insertion_idempotency_and_conflicting_fingerprint(tmp_path):
    ...
    again = repo.record_flow_evaluation(
        q,
        manifest,
        run_dir=run_dir,
        run_status=RunStatus.SUCCESS.value,
    ...
    with pytest.raises(RecordConflictError, match="different source fingerprint"):
        repo.record_flow_evaluation(
            changed, manifest, run_dir=run_dir, run_status=RunStatus.SUCCESS.value
        )
    ...
    with pytest.raises(RecordConflictError, match="different source fingerprint"):
        repo.record_flow_evaluation(
            changed_unserialized, manifest, run_dir=run_dir, run_status=RunStatus.SUCCESS.value
        )
...
def test_05_full_canonical_qor_and_nullable_unknown_fields_are_preserved(tmp_path):
    ...
    unknown = _qor(
        run_id="unknown", setup_ns=None, hold_ns=None, area=None, area_proxy=None, power=None
    )
    manifest, run_dir = _manifest(tmp_path, "unknown")
    repo.record_flow_evaluation(
        unknown, manifest, run_dir=run_dir, run_status=RunStatus.BLOCKED.value
    )
...
def test_06_power_status_parser_provenance_and_unknown_are_separate(tmp_path):
    ...
    q.raw_reports["power"] = {
        ...
        "configured_producer": "openroad_opensta",
        "producer_version": "2026.1",
        "tool_version": "OpenSTA 2.5",
        "original_unit": "mW",
        "normalized_unit": "W",
        "scenario_id": "SLOW",
        "mode": "functional",
        "corner": "slow",
        "report_path": "configured_power_report.rpt",
        "sha256": "a" * 64,
        "parsing_status": PowerParseStatus.AVAILABLE.value,
        "reported_internal_w": 0.0,
        "reported_switching_w": 0.0,
...
def test_08_candidate_session_constraint_identity_and_lineage(tmp_path):
    ...
    baseline = Candidate(
        id="C000",
        constraint_set=cset,
        qor=_qor(run_id="", candidate_id="C000"),
        decision=CandidateDecision.FINAL,
        hard_feasible=True,
    )
    child = Candidate(
        id="C001",
        ...
        generation=1,
        constraint_set=cset,
        generated_changes=["uncertainty+0.1"],
        mutated_constraint_ids=["U1"],
        qor=_qor(run_id="", candidate_id="C001"),
        hard_feasible=True,
    )
    session = OptimizationResult(baseline=baseline, final=child, all_candidates=[baseline, child])
    assert (
        repo.record_optimizer_session(session, session_id="session-a", project_name="proj")
        == "session-a"
    )
    ...
    assert candidate["mutations"] == [
        {"ordinal": 0, "change_label": "uncertainty+0.1", "mutated_constraint_id": "U1"}
    ]
    assert [item["candidate_id"] for item in repo.candidate_lineage("session-a", "C001")] == [
        "C000",
        "C001",
    ]
...
def test_09_mcmm_scenarios_remain_distinct_and_aggregate_is_conservative(tmp_path):
    ...
    repo.record_optimizer_session(
        OptimizationResult(baseline=candidate, final=candidate, all_candidates=[candidate]),
        session_id="mcmm-session",
    )
    slow = _qor(
        run_id="", candidate_id="C000", scenario="SLOW", setup_ns=1.0, area=100.0, power=1e-3
    )
    fast = _qor(
        run_id="", candidate_id="C000", scenario="FAST", setup_ns=0.5, area=101.0, power=None
    )
    result = MCMMResult(candidate_id="C000", active_scenario_ids=["SLOW", "FAST"])
    result.scenario_results["SLOW"] = ScenarioQoR(
        candidate_id="C000", scenario_id="SLOW", mode="functional", corner="slow", qor=slow
    )
    result.scenario_results["FAST"] = ScenarioQoR(
        candidate_id="C000", scenario_id="FAST", mode="test", corner="fast", qor=fast
    )
    global_feasibility(result)
    aggregate_objectives(result)
    mcmm_id = repo.record_mcmm_aggregate(
        result, session_id="mcmm-session", candidate_id="C000", candidate=candidate
    )
...
def test_10_mcmm_explicit_unknown_aggregate_and_scenario_filter(tmp_path):
    ...
    assert [item["evaluation"]["evaluation_id"] for item in repo.list_runs(scenario_id="FAST")] == [
        "fast"
    ]
    c = Candidate(id="C", constraint_set=ConstraintSet(name="x"))
    repo.record_optimizer_session(
        OptimizationResult(baseline=c, final=c, all_candidates=[c]), session_id="unknown-mcmm"
    )
    m = MCMMResult(candidate_id="C", active_scenario_ids=[])
    m.objectives["area"] = ObjectiveAggregate(
        name="area",
        value=None,
        unknown=True,
        incomparable=True,
    ...
    assert (
        ...
        and objective["unknown_value"] == 1
        and objective["incomparable"] == 1
    )
...
def test_12_transaction_rollback_leaves_no_partial_graph(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        repo,
        "_insert_power_evidence",
        lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("simulated insert failure")),
    )
    with pytest.raises(RuntimeError, match="simulated"):
        repo.record_flow_evaluation(q, manifest, run_dir=run_dir, run_status="SUCCESS")
    conn = sqlite3.connect(repo.db_path)
...
def test_13_legacy_import_is_explicit_idempotent_and_detects_conflicts(tmp_path):
    ...
    run_dir = output / "runs" / "legacy"
    run_dir.mkdir(parents=True)
    summary = _qor(
        run_id="legacy", candidate_id="OLD", setup_ns=None, area=None, power=None
    ).summary()
    summary.pop("cell_count")
    ...
    changed = json.loads((run_dir / "qor.json").read_text(encoding="utf-8"))
    changed["setup_wns_ns"] = 9.0
...
def test_14_legacy_import_preserves_power_provenance_and_missing_fields(tmp_path):
    output = tmp_path / "legacy-power"
    run_dir = output / "runs" / "legacy-power"
    run_dir.mkdir(parents=True)
    summary = _qor(run_id="legacy-power", power=None).summary()
    summary["power_provenance"] = {
        "parsing_status": "MALFORMED",
        "sha256": "deadbeef",
        "diagnostics": ["bad"],
    }
    (run_dir / "qor.json").write_text(json.dumps(summary), encoding="utf-8")
    (run_dir / "run_manifest.json").write_text(
        json.dumps(RunManifest().to_dict()), encoding="utf-8"
    )
    repo = SQLiteQoRRepository.for_output_dir(output)
    repo.import_legacy_artifacts()
    record = repo.get_run("legacy-power")
    assert (
        record["qor"]["power_total_w"] is None
        and record["power_evidence"]["parser_status"] == "MALFORMED"
    )


def test_15_two_connection_wal_read_safety_and_cache_key_is_passive(tmp_path):
    repo = _repo(tmp_path)
    _record(repo, tmp_path, run_id="concurrent")
    writer = repo._connect()
...
def test_16_database_failure_preserves_mock_qor_and_artifacts_when_explicitly_supplied(tmp_path):
    ...
    result = run_flow(
        Config(),
        ConstraintSet(name="top"),
        "# sdc\n",
        "COMPLETE",
        [],
        backend="mock",
        run_id="failure-safe",
        qor_repository=FailingRepository(),
    )
    ...
    assert (Path(result["run_dir"]) / "qor.json").is_file() and (
        Path(result["run_dir"]) / "run_manifest.json"
    ).is_file()


def test_17_cli_history_queries_and_json_output_are_deterministic(tmp_path):
    repo = _repo(tmp_path)
    _record(repo, tmp_path, run_id="cli", power=1.0)
    runner = CliRunner()
    ...
    structured = runner.invoke(
        app, ["history", "--output-dir", output_dir, "--best", "power", "--json"]
    )
    ...
    assert (
        ...
        and json.loads(structured.output)[0]["evaluation"]["evaluation_id"] == "cli"
    )
...
def test_18_cli_explicit_legacy_import_never_executes_eda_or_cache(tmp_path):
        ...
        json.dumps(
            {
                ...
                "changes": ["baseline"],
                "mutated_constraints": [],
                "qor": {"setup_wns_ns": 1.0, "power_status": "UNAVAILABLE"},
                "mcmm": {
                    "candidate_id": "OLD",
                    "active_scenarios": ["slow"],
                    "feasible": True,
                    "infeasible": False,
                    "blocked": False,
                    "invalid": False,
                    "global_status": "feasible",
                    "global_reason": "all pass",
                    "limiting_scenarios": ["slow"],
                    "objectives": {},
                    "scenario_results": {
                        "slow": {
                            "scenario_id": "slow",
    ...
    (output / "optimizer_state.json").write_text(
        json.dumps({"candidates": [{"id": "IGNORED"}]}), encoding="utf-8"
    )
    result = CliRunner().invoke(
        app, ["history", "--output-dir", str(output), "--import-legacy", "--json"]
    )
    payload = json.loads(result.output)
    assert (
        result.exit_code == 0
        and payload["imported"] == []
        and "No run directory" in payload["diagnostics"][0]
    )
    ...
    assert (
        imported.list_mcmm_scenarios(session_id=legacy_session, candidate_id="OLD")[0][
            "scenario_id"
        ]
        == "slow"
    )
    # Without JSONL the state file is a deterministic fallback.
    fallback = tmp_path / "state-fallback"
    fallback.mkdir()
    (fallback / "optimizer_state.json").write_text(
        json.dumps(
            {
                "candidates": [
                    {
    ...
    assert (
        imported_state.get_candidate(state_result["optimizer_sessions"][0]["session_id"], "STATE")
        is not None
    )
...
def test_20_flow_record_keeps_existing_constraint_set_hash(tmp_path):
    ...
    repo.record_flow_evaluation(
        q, manifest, run_dir=run_dir, run_status="SUCCESS", constraint_set=cset
    )
...
def test_21_session_candidate_ids_are_scoped_not_globally_inferred(tmp_path):
    ...
    for sid in ("one", "two"):
        ...
        repo.record_optimizer_session(
            OptimizationResult(baseline=c, final=c, all_candidates=[c]), session_id=sid
        )
...
def test_22_optimizer_session_conflict_is_explicit(tmp_path):
    ...
    changed = Candidate(
        id="C000", constraint_set=ConstraintSet(name="b"), generated_changes=["changed evidence"]
    )
    with pytest.raises(RecordConflictError, match="session"):
        repo.record_optimizer_session(
            OptimizationResult(baseline=changed, final=changed, all_candidates=[changed]),
...
def test_23_physical_flow_evaluation_can_link_to_session_candidate(tmp_path):
    ...
    repo.record_optimizer_session(
        OptimizationResult(baseline=c, final=c, all_candidates=[c]), session_id="link"
    )
    ...
    repo.record_optimizer_session(
        OptimizationResult(baseline=second, final=second, all_candidates=[second]),
        session_id="link-two",
    )
...
def test_24_list_runs_filters_status_candidate_scenario_and_constraint_hash(tmp_path):
    ...
    assert [item["evaluation"]["run_id"] for item in repo.list_runs(candidate_id="A")] == [
        "filter-a"
    ]
    assert [item["evaluation"]["run_id"] for item in repo.list_runs(scenario_id="S1")] == [
        "filter-a"
    ]
    assert [item["evaluation"]["run_id"] for item in repo.list_runs(run_status="SUCCESS")] == [
        "filter-a"
    ]
    assert (
        repo.find_by_constraint_set(stable_hash_cset(cset))[0]["evaluation"]["run_id"] == "filter-a"
    )
...
def test_27_sql_null_is_not_numeric_zero_for_unknown_metrics(tmp_path):
    ...
    conn = sqlite3.connect(repo.db_path)
    values = conn.execute(
        "SELECT setup_wns_s, power_total_w FROM qor_measurements WHERE evaluation_id='nullable'"
    ).fetchone()
...
def test_29_constraint_snapshot_is_a_locator_not_a_database_copy(tmp_path):
    ...
    repo.record_flow_evaluation(q, m, run_dir=d, run_status="SUCCESS", constraint_set=cset)
    conn = sqlite3.connect(repo.db_path)
    locator, digest = conn.execute(
        "SELECT snapshot_artifact_ref, snapshot_sha256 FROM constraint_sets"
    ).fetchone()
    assert locator == str(snapshot) and digest == hash_file(snapshot)
    assert (
        "constraints"
        not in conn.execute(
            "SELECT sql FROM sqlite_master WHERE name='constraint_sets'"
        ).fetchone()[0]
    )
...
def test_30_mcmm_graph_transaction_rolls_back_on_member_failure(tmp_path, monkeypatch):
    ...
    repo.record_optimizer_session(
        OptimizationResult(baseline=c, final=c, all_candidates=[c]), session_id="rollback-mcmm"
    )
    m = MCMMResult(candidate_id="C", active_scenario_ids=["S"])
    m.scenario_results["S"] = ScenarioQoR(
        candidate_id="C", scenario_id="S", qor=_qor(run_id="", candidate_id="C")
    )
...
def test_35_history_cli_candidate_session_and_selector_validation(tmp_path):
    ...
    c = Candidate(
        id="C000",
        constraint_set=ConstraintSet(name="cli"),
        qor=_qor(run_id="", candidate_id="C000"),
    )
    repo.record_optimizer_session(
        OptimizationResult(baseline=c, final=c, all_candidates=[c]), session_id="cli-session"
    )
    runner = CliRunner()
    ...
    assert (
        result.exit_code == 0
        and json.loads(result.output)["candidate"]["candidate_key"] == "cli-session:C000"
    )
...
def test_36_cache_is_stored_as_evidence_and_query_never_calls_flow_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        flow,
        "_find_cached_run",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("cache used")),
    )
...
def test_37_repeated_identical_candidate_and_mcmm_queries_are_deterministic(tmp_path):
    ...
    repo.record_optimizer_session(
        OptimizationResult(baseline=c, final=c, all_candidates=[c]), session_id="repeat"
    )
    m = MCMMResult(candidate_id="C", active_scenario_ids=[])
