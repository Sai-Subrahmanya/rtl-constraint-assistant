...
from rca.sdc.generation.tcl_quote import format_ns, tcl_quote, tcl_quote_list
...
def _gen(cs, backend="generic", mode="balanced", with_provenance_=False):
    return get_backend(backend).generate(
        cs, design_name="t", mode=mode, with_provenance=with_provenance_
    )
...
class TestIODelay:
    ...
    def test_clock_uncertainty_both_unqualified(self):
        c = _mk(
            ...
            target_objects=["clk"],
            clock_refs=["clk"],
            values={"uncertainty": 0.1e-9, "min_max": "both", "setup_hold": "both"},
        )
        r = _gen(_cset(c))
        assert r.status == GenerationStatus.COMPLETE
    ...
    def test_clock_latency_both_unqualified(self):
        c = _mk(
            ...
            target_objects=["clk"],
            clock_refs=["clk"],
            values={"latency": 0.3e-9, "min_max": "both", "setup_hold": "both"},
        )
        r = _gen(_cset(c))
        assert r.status == GenerationStatus.COMPLETE
...
class TestClockGroups:
    def test_groups_preserved(self):
        c = _mk(
            ...
            values={"relationship": "asynchronous", "groups": [["clk_a", "clk_b"], ["clk_c"]]},
        )
        r = _gen(_cset(c))
        assert r.status == GenerationStatus.COMPLETE
    ...
    def test_deterministic_order(self):
        constraints = [
            _mk(
                ...
                path_selector=(
                    PathSelector(from_set=["a"], to_set=["b"])
                    if t
                    in (
...
class TestBackendCapabilities:
    def test_cadence_blocks_edge_shift(self):
        ...
        assert any(
            d.severity == "ERROR" and "edge_shift" in d.message.lower() for d in r.diagnostics
        )
        # Result diagnostic is recorded for G1
        assert any(d.constraint_id == "G1" and d.severity == "ERROR" for d in r.diagnostics)
...
class TestResultStatus:
    ...
    def test_skipped_constraint_reflected_in_stats(self):
        ...
        c2 = _mk(
            id="BAD", type=ConstraintType.SET_INPUT_DELAY, target_objects=["d"], values={}
        )  # missing clock + delay
        r = _gen(_cset(c1, c2))
...
class TestRoundtrip:
    ...
    def test_io_rise_fall_preserved(self):
        sdc = (
            "set_input_delay -clock clk -rise -max 1 [get_ports d]\n"
    ...
    def test_generated_multiply(self):
        sdc = (
            "create_generated_clock -name gp -source [get_pins U/Q] "
    ...
    def test_generated_edges_and_shift_supported(self):
        ...
        sdc = (
            "create_generated_clock -name g -source [get_pins U/Q] "
    ...
    def test_semantic_equivalence_io_both(self):
        ...
        assert any(
            cc.values.get("min_max") == "max" and cc.values.get("edge") == "rise" for cc in ios
        )
        assert any(
            cc.values.get("min_max") == "max" and cc.values.get("edge") == "fall" for cc in ios
        )
