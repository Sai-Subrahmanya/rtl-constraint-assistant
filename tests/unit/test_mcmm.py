...
def _qor(
    *,
    ...
    return QoRResult(
        ...
        setup_tns=(
            (setup_tns if setup_tns is not None else 0.0) * 1e-9 if wns_ns is not None else None
        ),
        hold_tns=(
            (hold_tns if hold_tns is not None else 0.0) * 1e-9 if hold_ns is not None else None
        ),
        ...
        power=power,
        power_status=(
            PowerStatus.AVAILABLE.value if power is not None else PowerStatus.UNAVAILABLE.value
        ),
...
def _sqor(sid, **qkw):
    ...
    return ScenarioQoR(
        candidate_id="C0",
        scenario_id=sid,
        mode=qkw.get("mode", "functional"),
        corner=qkw.get("corner", "slow"),
        name=f"{qkw.get('mode', 'functional')}_{qkw.get('corner', 'slow')}",
        qor=q,
        backend="mock",
        tool="mock",
        tool_version="0.1",
...
def test_per_candidate_evaluation_of_every_scenario():
    ...
    ev = MCMMEvaluator(mat, evaluate_scenario=evaluate_scenario, base_cset=cs, name="mock")
...
def test_fixed_constraint_immutability_in_mcmm():
    ...
    for c in cands:
        ...
        assert not any(
            cid.startswith("CLK") and "UNC" not in cid for cid in c.mutated_constraint_ids
        )
...
def test_mcmm_pareto_not_superior_via_improving_one_scenario():
    ...
    a_ss = {"S1": _make("S1", wns_ns=0.9, hold_ns=0.2), "S2": _make("S2", wns_ns=0.2, hold_ns=0.2)}
    b_ss = {"S1": _make("S1", wns_ns=0.5, hold_ns=0.2), "S2": _make("S2", wns_ns=0.5, hold_ns=0.2)}
...
def test_mcmm_pareto_dominates_all_scenarios():
    a_ss = {
        "S1": _make("S1", wns_ns=0.9, hold_ns=0.3, area=90.0, power=50.0),
        "S2": _make("S2", wns_ns=0.8, hold_ns=0.3, area=90.0, power=50.0),
    }
    b_ss = {
        "S1": _make("S1", wns_ns=0.5, hold_ns=0.2, area=120.0, power=80.0),
...
def test_mcmm_final_selection_prefers_better_global_timing():
    a = _mcmm_candidate(
        "A",
        {"S1": _make("S1", wns_ns=0.9, hold_ns=0.3), "S2": _make("S2", wns_ns=0.8, hold_ns=0.3)},
    )
    b = _mcmm_candidate(
        "B",
        {"S1": _make("S1", wns_ns=0.4, hold_ns=0.2), "S2": _make("S2", wns_ns=0.4, hold_ns=0.2)},
    )
    front = mcmm_pareto_front([a, b])
    priorities = {"timing": Priority.HIGH, "area": Priority.MEDIUM, "power": Priority.MEDIUM}
...
def test_mcmm_select_final_uses_margin_tiebreak_when_equal():
    a = _mcmm_candidate(
        "A",
        {"S1": _make("S1", wns_ns=0.5, hold_ns=0.2), "S2": _make("S2", wns_ns=0.5, hold_ns=0.2)},
    )
    b = _mcmm_candidate(
        "B",
        {"S1": _make("S1", wns_ns=0.5, hold_ns=0.2), "S2": _make("S2", wns_ns=0.5, hold_ns=0.2)},
...
def test_scenario_specific_sdc():
    ...
    sdc_s1 = backend.generate(cs, design_name="m", mode=SafeMode.BALANCED, scenario="S1").text
    sdc_s2 = backend.generate(cs, design_name="m", mode=SafeMode.BALANCED, scenario="S2").text
...
def test_repeated_evaluation_determinism():
    ...
    for sid in res_a.active_scenario_ids:
        assert (
            res_a.scenario_results[sid].qor.setup_wns == res_b.scenario_results[sid].qor.setup_wns
        )
...
def test_mcmm_margin_tiebreak_affects_final_selection():
    ...
    a = _mcmm_candidate(
        "A",
        {
            "S1": _make("S1", wns_ns=0.5, hold_ns=0.2, area=100.0, power=50.0),
            "S2": _make("S2", wns_ns=0.5, hold_ns=0.2, area=100.0, power=50.0),
        },
    )
    b = _mcmm_candidate(
        "B",
...
def test_cache_key_different_environment_different_key():
    cs = _cset()
    k1 = scenario_cache_key(
        Scenario(id="A", mode="functional", corner="slow", environment={"temp": 125}), cs
    )
    k2 = scenario_cache_key(
        Scenario(id="A", mode="functional", corner="slow", environment={"temp": -40}), cs
    )
    assert k1 != k2
...
def test_mcmm_pareto_unknown_blocks_dominance():
    ...
    a = _mcmm_candidate(
        "A",
        {
            "S1": _make("S1", wns_ns=0.8, hold_ns=0.3, area=90.0, power=50.0),
            "S2": _make("S2", wns_ns=0.8, hold_ns=0.3, area=90.0, power=50.0),
        },
    )
    b = _mcmm_candidate(
        "B",
        {
            "S1": _make("S1", wns_ns=0.5, hold_ns=0.2, area=90.0),
...
def test_mcmm_pareto_area_incomparability_blocks_dominance():
    ...
    a = _mcmm_candidate(
        "A",
        {
            "S1": _make("S1", wns_ns=0.9, hold_ns=0.3, area=100.0, area_proxy=None),
            "S2": _make("S2", wns_ns=0.9, hold_ns=0.3, area=100.0, area_proxy=None),
        },
    )
    b = _mcmm_candidate(
        "B",
        {
            "S1": _make("S1", wns_ns=0.5, hold_ns=0.2, area=None, area_proxy=100.0),
            "S2": _make("S2", wns_ns=0.5, hold_ns=0.2, area=None, area_proxy=100.0),
        },
    )
    assert not mcmm_is_dominating(a, b)
    assert not mcmm_is_dominating(b, a)
...
def test_mcmm_pareto_only_globally_feasible_enter():
    ...
    b = _mcmm_candidate(
        "B",
        {"S1": _make("S1", wns_ns=0.5, hold_ns=0.2), "S2": _make("S2", wns_ns=-0.1, hold_ns=0.2)},
        hard=False,
    )
...
def test_sdc_empty_scenario_ids_applies_to_all():
    ...
    cs.add(u)
    backend = get_backend("generic")
    s1 = backend.generate(cs, design_name="m", mode=SafeMode.BALANCED, scenario="S1").text
    s2 = backend.generate(cs, design_name="m", mode=SafeMode.BALANCED, scenario="S2").text
...
def test_sdc_nonempty_scenario_ids_only_listed():
    ...
    u2 = Constraint(
        ...
        target_objects=["clk"],
        clock_refs=["clk"],
        values={"uncertainty": 0.05e-9},
        scenario_ids=["GHOST"],
    )
    cs.add(u2)
    s1b = backend.generate(cs, design_name="m", mode=SafeMode.BALANCED, scenario="S1").text
...
def test_sdc_fixed_constraints_immutable_and_provenance_intact():
    ...
    gen = backend.generate(
        cs, design_name="m", mode=SafeMode.BALANCED, with_provenance=True, scenario="S1"
    )
