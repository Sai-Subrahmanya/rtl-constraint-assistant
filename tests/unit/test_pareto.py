...
def _make(id, wns_ns=0.5, hold_ns=0.2, area=100, power=100, setup_viol=0, hold_viol=0):
    q = QoRResult(
        setup_wns=wns_ns * 1e-9,
        hold_wns=hold_ns * 1e-9,
    ...
    return Candidate(
        id=id,
        qor=q,
        decision=CandidateDecision.EVALUATED,
...
def test_non_domination_tradeoff():
    ...
    _make("A", 0.8, 0.3, 100, 100)
    _make("B", 0.2, 0.18, 102, 101)
...
def test_feasible_pass_with_margin():
    fr = classify_feasibility(
        _qor(wns_ns=0.5, hold_ns=0.2), required_setup_ns=0.1, required_hold_ns=0.05
    )
    assert fr.feasible
...
def test_required_margin_rejects_tight_slack():
    ...
    fr = classify_feasibility(_qor(wns_ns=0.08, hold_ns=0.2), required_setup_ns=0.1)
    assert not fr.feasible
    assert fr.infeasible_reason == "setup_violation"
...
def test_margin_headroom_above_required():
    ...
    m = compute_margin(
        q,
        required_setup_ns=0.0,
        required_hold_ns=0.0,
        baseline_setup_wns=1.0e-9,
        baseline_hold_wns=0.3e-9,
    )
...
def test_margin_below_required_is_infeasible():
    fr = classify_feasibility(_qor(wns_ns=0.05, hold_ns=0.1), required_setup_ns=0.1)
    assert not fr.feasible
...
def _make_cset_with_fixed():
    ...
    u = cset.create_clock_uncertainty("clk", 0.1e-9, source_kind=SourceKind.INFERENCE)
...
def test_deterministic_candidate_generation():
    ...
    cs1 = _make_cset_with_fixed()
    b1 = Candidate(id="C0", constraint_set=cs1)
    cs2 = _make_cset_with_fixed()
    b2 = Candidate(id="C0", constraint_set=cs2)
...
def test_baseline_preserved_when_optimal():
    ...
    def _eval(cand, work_dir):
        return {
            "qor": copy.deepcopy(baseline_qor),
            "cache_key": "K" + cand.id,
            "cache_status": "MISS",
            "run_id": "r" + cand.id,
        }
...
def test_optimizer_returns_structured_result_with_provenance(tmp_path):
    ...
    def _eval(cand, work_dir):
        q = _qor(wns_ns=0.5 - 0.02 * cand.generation, hold_ns=0.2, area=100.0 - 5 * cand.generation)
        return {
            "qor": q,
            "cache_key": "K" + cand.id,
            "cache_status": "MISS",
    ...
    res = opt.run(_make_cset_with_fixed(), baseline_qor=_qor(wns_ns=0.5, hold_ns=0.2, area=100.0))
...
def test_cache_hit_returns_same_qor_and_counts_hit(tmp_path):
    ...
    def _eval(cand, work_dir):
        ...
        if not hasattr(_eval, "_seen"):
            _eval._seen = set()
        key = cand.constraint_model_hash or cand.id
        if key in _eval._seen:
            return {
                "qor": _qor(wns_ns=0.5, hold_ns=0.2, area=100.0),
                "cache_key": key,
                "cache_status": "HIT",
                "run_id": "r_hit",
            }
    ...
    res = opt.run(_make_cset_with_fixed(), baseline_qor=None)
    assert res.final is not None
...
def test_different_candidates_have_different_experiment_identity(tmp_path):
    ...
    def _eval(cand, work_dir):
        q = _qor(
            wns_ns=max(0.1, 0.5 - 0.05 * cand.generation),
            hold_ns=0.2,
...
def test_adversarial_high_slack_bad_area_vs_low_slack_good_area_both_pareto():
    ...
    b = _cand(
        "B", wns_ns=0.2, hold_ns=-0.05, area=50.0, hard_feasible=False, reason="hold_violation"
    )
...
def test_margin_no_baseline_excess_yields_none_utilization():
    ...
    q = _qor(wns_ns=0.05, hold_ns=0.3)
    m = compute_margin(
        q, required_setup_ns=0.05, baseline_setup_wns=0.05e-9, baseline_hold_wns=0.3e-9
    )
    assert m["margin_utilization"] is None
...
def test_margin_zero_consumption_yields_zero_utilization():
    ...
    m = compute_margin(
        q, required_setup_ns=0.0, baseline_setup_wns=0.1e-9, baseline_hold_wns=0.3e-9
    )
...
def test_margin_hold_limited_caps_headroom():
    ...
    m = compute_margin(
        ...
        required_setup_ns=0.0,
        required_hold_ns=0.0,
        baseline_setup_wns=1e-9,
        baseline_hold_wns=0.2e-9,
    )
...
def _make_cset_with_two_uncertainties():
    ...
    cset.create_clock(
        name="clk2",
        period_seconds=20e-9,
        fixed=True,
        source_kind=SourceKind.USER,
        confidence=Confidence.HIGH,
    )
    u1 = cset.create_clock_uncertainty("clk", 0.05e-9, source_kind=SourceKind.INFERENCE)
    u1.opt_status = OptimizationStatus.TUNABLE
    u2 = cset.create_clock_uncertainty("clk2", 0.05e-9, source_kind=SourceKind.INFERENCE)
...
def test_one_mutation_per_candidate_baseline_strategy():
    ...
    for c in cands:
        assert len(c.mutated_constraint_ids) == 1, f"{c.id} mutated {c.mutated_constraint_ids}"
...
def test_changing_u1_leaves_u2_unchanged():
    ...
    class _Cfg:
        ...
        analysis = _A()
        _config_path = "/tmp/iso_mut/p.yaml"
        optimization = OptimizationConfig(
            enabled=True,
            max_iterations=8,
            max_eda_runs=20,
    ...
    for c in cset:
        if c.id == u2:
            u2_initial = float(c.values.get("uncertainty", 0.0))
    cands = generate_candidates(base, cset, _Cfg(), max_candidates=20)
...
def test_fixed_constraints_never_touched_in_one_mutation_strategy():
    ...
    class _Cfg:
        ...
        analysis = _A()
        _config_path = "/tmp/fix_imm/p.yaml"
        optimization = OptimizationConfig(
            enabled=True,
            max_iterations=4,
            max_eda_runs=10,
...
def test_semantic_hash_differs_on_opt_status():
    ...
    assert stable_hash_cset(_mk(OptimizationStatus.TUNABLE)) != stable_hash_cset(
        _mk(OptimizationStatus.FIXED)
    )
...
def test_blocked_candidate_not_pareto_eligible():
    c = _cand("BLK", wns_ns=0.5, hold_ns=0.2, blocked=True, hard_feasible=False, reason="blocked")
...
def test_hard_infeasible_cannot_dominate_or_be_final():
    ...
    priorities = {"timing": Priority.HIGH, "area": Priority.HIGH, "power": Priority.HIGH}
...
def test_more_margin_consumption_alone_does_not_dominate():
    ...
    a = _cand("A", wns_ns=0.9, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.1)
    b = _cand("B", wns_ns=0.5, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.5)
...
def test_margin_consumption_with_area_tradeoff_is_valid():
    ...
    a = _cand("A", wns_ns=0.9, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.1)
    b = _cand("B", wns_ns=0.5, hold_ns=0.5, area=90.0, hard_feasible=True, margin_util=0.5)
    assert not is_dom_new(a, b)
    assert not is_dom_new(b, a)
    front = pareto_front([a, b])
...
def test_margin_consumption_with_power_tradeoff_is_valid():
    a = _cand(
        "A", wns_ns=0.9, hold_ns=0.5, area=100.0, power=100.0, hard_feasible=True, margin_util=0.1
    )
    b = _cand(
        "B", wns_ns=0.5, hold_ns=0.5, area=100.0, power=50.0, hard_feasible=True, margin_util=0.5
    )
    assert not is_dom_new(a, b)
    assert not is_dom_new(b, a)
    front = pareto_front([a, b])
...
def test_margin_consumption_no_ppa_gain_no_preference():
    ...
    a = _cand("A", wns_ns=0.9, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.1)
    c = _cand("C", wns_ns=0.5, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.9)
...
def test_below_hard_floor_infeasible_regardless_of_margin():
    ...
    fr = classify_feasibility(q)
    assert not fr.feasible
    compute_margin(q, baseline_setup_wns=1e-9)
...
def test_select_final_prefers_lower_margin_when_ppa_equal():
    ...
    a = _cand("A", wns_ns=0.5, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.1)
    b = _cand("B", wns_ns=0.5, hold_ns=0.5, area=100.0, hard_feasible=True, margin_util=0.9)
    front = pareto_front([a, b])
    priorities = {"timing": Priority.HIGH, "area": Priority.MEDIUM, "power": Priority.LOW}
    final = select_final(front, a, priorities)
...
def test_canonical_hash_insertion_order_independent():
    from rca.constraint_model import Constraint, stable_hash_cset
...
def test_canonical_hash_differs_on_source_objects():
    ...
    def mk(srcs):
        ...
        c = Constraint(
            id="u",
            type=CT.SET_FALSE_PATH,
            source_objects=list(srcs),
            target_objects=["clk"],
            values={},
        )
        cs.add(c)
...
def test_canonical_hash_differs_on_through_objects():
    ...
    def mk(thr):
        ...
        c = Constraint(
            id="u",
            type=CT.SET_MULTICYCLE_PATH,
            source_objects=["a"],
            target_objects=["b"],
            through_objects=list(thr),
            values={"cycles": 2},
        )
        cs.add(c)
        return cs
...
def test_canonical_hash_differs_on_opt_status():
    from rca.constraint_model import Constraint, stable_hash_cset
    from rca.utils.enums import ConstraintType as CT
    from rca.utils.enums import OptimizationStatus
    ...
    assert stable_hash_cset(mk(OptimizationStatus.TUNABLE)) != stable_hash_cset(
        mk(OptimizationStatus.FIXED)
    )
...
def _make_cset_with_missing_value_constraint():
    ...
    u = cset.create_clock_uncertainty("clk", 0.05e-9, source_kind=SourceKind.INFERENCE)
    u.opt_status = OptimizationStatus.TUNABLE
...
def test_missing_value_does_not_create_zero_internal():
    ...
    for c in cands:
        for cc in c.constraint_set:
            if cc.id == bare_id:
                assert "delay" not in cc.values, "missing delay must not be invented"
...
def test_fixed_constraints_still_never_mutated():
    ...
    for c in cands:
        ...
        assert (
            ...
            or True
        )  # uncertainty clock-named ids are allowed; only CREATE_CLOCK period is fixed
...
def test_select_final_mixed_area_does_not_compare_numerically():
    ...
    priorities = {
        ...
        "area": Priority.HIGH,
        "power": Priority.LOW,
        "constraint_quality": Priority.LOW,
    }
    final = select_final(front, a, priorities)
...
def test_no_conversion_between_real_proxy_unknown_in_scalar():
    ...
    b = _q_cand("B", wns_ns=0.5, hold_ns=0.3, area=None, area_proxy=1.0)
    sa = scalar_score(
        a, a, {"timing": Priority.HIGH, "area": Priority.HIGH, "power": Priority.MEDIUM}
    )
    sb = scalar_score(
        b, a, {"timing": Priority.HIGH, "area": Priority.HIGH, "power": Priority.MEDIUM}
    )
