...
def _evidence(identifier: str, *, rule: str | None = None) -> Evidence:
    return Evidence(
        id=identifier,
        kind="structural",
        description=f"evidence {identifier}",
        source_objects=["clk"],
...
def _validation(*issues: ValidationIssue, status: str = "PASS") -> ValidationResult:
    return ValidationResult(
        ...
        coverage=CoverageReport(graph_available=True),
    )
...
def _accepted_context(*, knowledge: bool = False):
    ...
    candidate = next(
        item for item in advisory.candidates if item.decision == InferenceDecision.ACCEPTABLE
    )
    if knowledge:
        candidate = replace(
            candidate,
            knowledge_references=(
                {
                    "knowledge_item_id": "K-TEST",
                    "suggestion_id": "KS-TEST",
                    "trust_level": "VALIDATED",
                    "applicability": "APPLICABLE",
                    "match_kind": "semantic_exact",
                },
            ),
        )
        advisory = replace(
...
def test_user_imported_inferred_and_unknown_sources_are_not_relabelled():
    ...
    user.provenance = ProvenanceRecord(
        ...
        created_by="user",
        evidence=[_evidence("E-USER")],
        created_at=_EPOCH,
    )
    imported = Constraint(
        id="IMP-1",
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["imp_clk"],
        ...
        provenance=ProvenanceRecord(
            ...
            evidence=[
                Evidence(
                    id="E-KNOW",
                    kind="knowledge",
                    description="advisory knowledge reference",
                    source_objects=["know_clk"],
                    confidence=Confidence.MEDIUM,
                    rule_id="KNOWLEDGE-REUSE",
                    detail={"knowledge_item_id": "K-1"},
                    created_at=_EPOCH,
                )
            ],
        ),
    ...
    system = Constraint(
        id="SYS-1",
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["rtl_clk"],
        values={"name": "rtl_clk", "period": 12e-9, "source": "rtl_clk"},
        source_kind=SourceKind.RTL,
        provenance=ProvenanceRecord(
            source_kind=SourceKind.RTL, created_by="parser", created_at=_EPOCH
        ),
    )
    unknown = Constraint(
        id="UNK-1",
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["legacy_clk"],
...
def test_candidate_application_ucm_knowledge_and_evidence_chain_is_preserved_read_only():
    ...
    assert {event.kind for event in entry.events} >= {
        LineageEventKind.APPLICATION_ATTEMPTED,
        LineageEventKind.ACCEPTED,
        LineageEventKind.CREATED,
    }
...
def test_validation_and_formal_evidence_are_separate_from_constraint_source():
    ...
    exception.provenance = ProvenanceRecord(
        source_kind=SourceKind.USER, created_by="user", created_at=_EPOCH
    )
    issue = ValidationIssue(
        severity=Severity.INFO,
        category=ValidationCategory.EXCEPTION,
        code=ErrorCode.EXCEPTION_UNVERIFIED,
        message="Exception has no proof.",
...
def test_stale_candidate_and_receipt_are_preserved_without_removal_or_refresh():
    ...
    report = build_constraint_lineage(
        ...
        design=design,
        timing_graph=timing,
        inference_report=stale_advisory,
        application_receipts=(stale_receipt,),
    )
    entry = report.constraints[0]

    assert any(
        event.kind == LineageEventKind.BECAME_STALE and event.stale for event in entry.events
    )
    assert any(
        event.kind == LineageEventKind.BECAME_STALE and event.stale
...
def test_rejected_deferred_and_already_present_attempts_do_not_create_constraint_events():
    ...
    rejected = ConstraintApplicationResult(
        ...
        candidate_id="IC-REJECT",
        candidate_semantic_identity=None,
        decision=IntentDecisionKind.REJECT,
        status=ApplicationStatus.REJECTED,
        ucm_mutated=False,
        rejected_constraint_ids=("PROPOSAL",),
    )
    deferred = ConstraintApplicationResult(
        application_id="APP-DEFER",
        candidate_id="IC-DEFER",
        candidate_semantic_identity=None,
        decision=IntentDecisionKind.DEFER,
        status=ApplicationStatus.DEFERRED,
        ucm_mutated=False,
    )
    already = ConstraintApplicationResult(
        application_id="APP-PRESENT",
        candidate_id="IC-PRESENT",
        candidate_semantic_identity=None,
        decision=IntentDecisionKind.ALREADY_SATISFIED,
        status=ApplicationStatus.ALREADY_PRESENT,
        ucm_mutated=False,
        already_present_ids=(user.id,),
    ...
    assert not any(event.kind == LineageEventKind.CREATED for event in entry.events)
    attempts = {
        event.application_id: event.kind
        for event in report.application_attempts
...
def test_semantic_snapshot_diff_reuses_equivalence_for_unit_equivalence_changes_unknown_and_mcmm():
    ...
    assert any(
        item.kind == LineageChangeKind.UNCHANGED and item.before_constraint_id == clock.id
    ...
    assert any(
        item.kind == LineageChangeKind.MODIFIED
        and item.before_constraint_id == clock.id
        and any(field["field"] == "period" for field in item.semantic_field_changes)
        for item in changes
    )
    assert any(
        item.kind == LineageChangeKind.REMOVED and item.before_constraint_id == scoped.id
        for item in changes
    )
    assert any(
        item.kind == LineageChangeKind.ADDED and item.after_constraint_id == added.id
        for item in changes
    )
    assert any(
        item.kind == LineageChangeKind.UNKNOWN and item.before_constraint_id == "LOAD"
        for item in changes
    )


def test_mcmm_scope_readiness_change_and_engine_json_are_deterministic_without_mutation():
    design, timing, config = _context(
        _SOURCE,
        clocks=[
            {
                "name": "clk",
    ...
    after_readiness = assess_constraint_readiness(
        config, after, design, timing, validation=after_validation
    )
    before_json = before.to_canonical_json()
    after_json = after.to_canonical_json()
    ...
    before_readiness = ConstraintReadinessReport(
        ...
        requirements=(
            ReadinessRequirement(
                ...
                status=ReadinessStatus.BLOCKED,
                required=True,
                rationale="Existing validation evidence is blocking.",
            ),
        ),
    )
    after_readiness = ConstraintReadinessReport(
        ...
        requirements=(
            ReadinessRequirement(
                ...
                status=ReadinessStatus.READY,
                required=True,
                rationale="Existing validation evidence is no longer blocking.",
            ),
        ),
    )
