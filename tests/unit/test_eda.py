...
from rca.utils.hashing import hash_text, stable_hash
...
def _fake_tool(
    name: str, code: int = 0, stdout: str = "tool-version-1.0\n", stderr: str = ""
) -> Path:
...
def test_05_safe_run_uses_argv_not_shell(tmp):
    ...
    class _Stub(ToolBackend):
        ...
        def discover(self):
            return ToolInfo(vendor="x", tool="x", version="x")

        def run_sta(self, *a, **k):
            pass
...
def test_19_real_flow_blocked_without_tools(tmp):
    ...
    assert any("not found" in d or "Liberty" in d or "unavailable" in d for d in out["diagnostics"])
...
def _make_flow_run(
    ...
    *,
    ...
    return run_flow(
        ...
        sources=sources or [],
        defines=defines,
        include_dirs=includes and [Path(p) for p in includes],
        parameters=params,
        backend=backend,
        run_id=run_id,
...
def test_synth_script_identity_changes(tmp):
    ...
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        src = td / "top.v"
        src.write_text("module top(input clk);endmodule\n", encoding="utf-8")
        lib1 = td / "a.lib"
        lib1.write_text("library(a) { cell() { area:1; } }\n", encoding="utf-8")
        y = YosysBackend(executable=str(tmp / "_ny"))
        s1, sp1, n1 = y._build_script(
            [src], "top", [lib1], td, defines={}, include_dirs=[], parameters={}
        )
        s2, sp2, n2 = y._build_script(
            [src], "top", [], td / "no", defines={}, include_dirs=[], parameters={}
        )
...
def test_relative_paths_resolve_correctly(tmp):
    ...
    assert (
        _resolve_rel(run_dir, str((run_dir / "x.txt").resolve())) == (run_dir / "x.txt").resolve()
    )
...
def test_hold_policy_explicit_in_sta_script():
    ...
    tcl, _, rpts = ob._build_script(
        netlist=Path("/tmp/n.v"),
        sdc=Path("/tmp/s.sdc"),
...
def _make_complete_cfg(tmp, *, backend="yosys_opensta", libs=None, out=None):
    ...
    class _FakeFlow:
        ...
        def liberty_files(self):
            return [str(p) for p in libs_list]
...
def test_cache_hit_then_invalidation_then_restore_e2e(tmp):
    ...
    out1 = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="re1", sdc_text=sdc_text, libs=libs, sources=sources
    )
    ...
    out2 = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="re2", sdc_text=sdc_text, libs=libs, sources=sources
    )
    ...
    out3 = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="re3", sdc_text=sdc_text, libs=libs, sources=sources
    )
    ...
    out4 = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="re4", sdc_text=sdc_text, libs=libs, sources=sources
    )
...
def test_input_change_changes_key_but_output_tamper_does_not(tmp):
    ...
    out_a = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="ka", sdc_text=sdc_a, libs=libs, sources=sources
    )
    out_b = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="kb", sdc_text=sdc_b, libs=libs, sources=sources
    )
    ...
    net_a.write_text("X\n", encoding="utf-8")
    out_a2 = _run_once(
        out_dir, ybin=ybin, obin=obin, run_id="ka2", sdc_text=sdc_a, libs=libs, sources=sources
    )
