...
def _fp(cs, cid: str, fro=None, to=None, through=None, scenarios=None, fixed=False):
    ps = PathSelector(from_set=fro or [], to_set=to or [], through_set=[through] if through else [])
    c = Constraint(
        id=cid,
        type=ConstraintType.SET_FALSE_PATH,
        path_selector=ps,
        scenario_ids=list(scenarios or []),
...
def test_11_clock_group_interaction_recorded():
    ...
    _clock(cs, "clk_a")
    _clock(cs, "clk_b")
    cs.create_clock_groups(groups=[["clk_a"], ["clk_b"]], relationship="asynchronous")
    _fp(cs, "FP1", fro=["clk_a"], to=["clk_b"])
    tg = _make_tg(
        clocks=["clk_a", "clk_b"],
        paths=[
...
def test_12_cdc_sync_does_not_auto_verify():
    ...
    rep = verify_exceptions(cs, design=MINI_DESIGN, tg=tg, backend=ConservativeFormalBackend())
...
def test_19_provenance_survives():
    ...
    assert d["evidence"]["selector"] == {"from": ["ra/Q"], "to": ["rb/D"], "through": []}
...
def test_21_deterministic_verification_results():
    ...
    mock = MockFormalBackend(
        verdicts={
            "FP1": VerificationResult(
                constraint_id="FP1", status=VerificationStatus.VERIFIED, message="ok"
            )
        }
    )
...
def test_22_analysis_does_not_mutate_ucm():
    ...
    before = [
        (
            ...
            tuple(sorted(c.scenario_ids or [])),
        )
        for c in cs
    ]
...
def test_32_backend_error_gives_error_status():
    ...
    rep = verify_exceptions(cs, design=MINI_DESIGN, tg=tg, backend=ExplodingBackend())
    r = _find(rep, "FP1")
...
def test_33_emittable_helper_filters_by_mode():
    ...
    mock = MockFormalBackend(
        verdicts={
            "FP_VERIFIED": VerificationResult(
                constraint_id="FP_VERIFIED", status=VerificationStatus.VERIFIED, tool="mock"
            ),
        }
    )
    rep = verify_exceptions(cs, design=MINI_DESIGN, tg=tg, backend=mock)
...
def test_43_user_rejected_blocks_emission():
    ...
    mock = MockFormalBackend(
        verdicts={
            "FP1": VerificationResult(
                constraint_id="FP1", status=VerificationStatus.VERIFIED, tool="mock"
            ),
        }
    )
    rep = verify_exceptions(cs, design=MINI_DESIGN, tg=tg, backend=mock, user_rejected_ids={"FP1"})
    r = _find(rep, "FP1")
...
def test_51_two_ordered_through_stages_match():
    ...
    fp1 = Constraint(
        id="FP1",
        type=ConstraintType.SET_FALSE_PATH,
        path_selector=PathSelector(from_set=["A"], to_set=["Y"], through_set=[["B"], ["C"]]),
    )
    cs.add(fp1)
    tg = _make_tg(
        clocks=["clk"],
...
def test_52_reversed_through_stages_do_not_match():
    ...
    fp1 = Constraint(
        id="FP1",
        type=ConstraintType.SET_FALSE_PATH,
        path_selector=PathSelector(from_set=["A"], to_set=["Y"], through_set=[["C"], ["B"]]),
    )
    cs.add(fp1)
    tg = _make_tg(
        clocks=["clk"],
...
def test_53_multi_object_through_stage_or():
    ...
    fp1 = Constraint(
        id="FP1",
        type=ConstraintType.SET_FALSE_PATH,
        path_selector=PathSelector(from_set=["A"], to_set=["Y"], through_set=[["B", "D"]]),
    )
    cs.add(fp1)
    tg = _make_tg(
        clocks=["clk"],
...
def test_57_multi_through_longer_path():
    ...
    fp1 = Constraint(
        id="FP1",
        type=ConstraintType.SET_FALSE_PATH,
        path_selector=PathSelector(from_set=["A"], to_set=["Y"], through_set=[["B"], ["D"], ["F"]]),
    )
    cs.add(fp1)
    tg = _make_tg(
        clocks=["clk"],
...
def test_60_through_changes_blast_radius():
    ...
    fp_narrow = Constraint(
        id="FP_NARROW",
        type=ConstraintType.SET_FALSE_PATH,
        path_selector=PathSelector(from_set=["clk"], to_set=["clk"], through_set=[["B"]]),
    )
    cs.add(fp_wide)
    cs.add(fp_narrow)
    paths = [
