...
def _raw(root: Path) -> dict:
    return {
        "schema_version": "1.0",
        "project": {"name": "workflow", "top": "top"},
        "sources": {"files": ["rtl/top.sv"], "include_dirs": [], "defines": []},
        "workflow": {
            "ucm_snapshot": "reviewed-ucm.json",
            "knowledge_sources": ["knowledge.json"],
            "inference_policy": {"safe": True},
            "application_policy": {"explicit": True},
            "validation_policy": {"backend": "generic"},
            "coverage_policy": {"require_complete": True},
            "readiness_policy": {},
            "review_policy": {},
            "release_policy": {},
            "handoff_policy": {},
            "release_package_dir": "artifacts/release",
...
def test_workflow_configuration_loads_validates_and_resolves_references(tmp_path: Path):
    ...
    path.write_text(yaml.safe_dump(_raw(tmp_path), sort_keys=False), encoding="utf-8")
...
def test_engineering_config_identity_is_portable_across_project_roots(tmp_path: Path):
    ...
    for root in (left, right):
        (root / "project.yaml").write_text(
            yaml.safe_dump(_raw(root), sort_keys=False), encoding="utf-8"
        )
    assert (
        load_config(left / "project.yaml").engineering_dict()
        == load_config(right / "project.yaml").engineering_dict()
    )
...
def test_invalid_workflow_target_fails_with_clear_validation_error(tmp_path: Path, bad: str):
    ...
    data = _raw(tmp_path)
    data["workflow"]["handoff_target"] = bad
...
def test_unknown_workflow_fields_and_nonobject_policies_fail_closed(tmp_path: Path):
    ...
    data = _raw(tmp_path)
    data["workflow"]["unsafe_unknown"] = True
    path = tmp_path / "unknown.yaml"
    path.write_text(yaml.safe_dump(data), encoding="utf-8")
    with pytest.raises(ValueError):
        load_config(path)
    data = _raw(tmp_path)
    data["workflow"]["release_policy"] = ["not", "a", "mapping"]
    path.write_text(yaml.safe_dump(data), encoding="utf-8")
    with pytest.raises(ValueError):
        load_config(path)
...
def test_workflow_config_json_yaml_round_trip_is_deterministic(tmp_path: Path):
    ...
    assert (
        WorkflowConfig.model_validate(config.workflow.model_dump()).model_dump()
        == config.workflow.model_dump()
    )
    out = tmp_path / "out.yaml"
    write_config(config, out)
    loaded = load_config(out)
    assert json.dumps(loaded.engineering_dict(), sort_keys=True) == json.dumps(
        config.engineering_dict(), sort_keys=True
    )
