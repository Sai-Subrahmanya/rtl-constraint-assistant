...
def _run(
    ...
    tg = TimingGraph.build(
        d,
        user_clocks=uc_dicts,
        user_relationships=[
            r.model_dump() if hasattr(r, "model_dump") else r for r in (user_rels or [])
        ],
    )
    cfg = ProjectConfig(project=ProjectInfo(name="m", top=top, rtl_files=rtl_files or ["x.sv"]))
    if user_clocks:
        for uc in user_clocks:
            cfg.constraints.user.clocks.append(
                UserClockSpec(
                    name=uc["name"],
                    period=uc.get("period"),
                    fixed=uc.get("fixed", True),
                    port=uc.get("port"),
                    uncertainty=uc.get("uncertainty"),
                )
            )
    for name, spec in (user_inputs or {}).items():
        cfg.constraints.user.io.inputs[name] = (
            spec if isinstance(spec, IOPortSpec) else IOPortSpec(**spec)
        )
    for name, spec in (user_outputs or {}).items():
        cfg.constraints.user.io.outputs[name] = (
            spec if isinstance(spec, IOPortSpec) else IOPortSpec(**spec)
        )
...
def test_02_clock_name_alone_does_not_create_clock():
    ...
    sv = "module m(input clk, d, output reg q); always_ff @(posedge d) q<=clk; endmodule"
    _, tg, _, cset, _, rpt = _run(sv)
    assert _by_type(cset, ConstraintType.CREATE_CLOCK) == []
    ...
    [
        ...
        if "name" in w.get("message", "").lower() or "looks" in w.get("message", "").lower()
    ]
    # either no clock created for the name-hint
    assert not any(
        "create_clock" in str(pc)
...
def test_06b_input_single_justified_clock_emits_when_period_known():
    sv = "module m(input clk, d, output reg q); always_ff @(posedge clk) q<=d; endmodule"
    _, _, _, cset, _, rpt = _run(
        sv,
        user_clocks=[{"name": "clk", "period": "10ns", "fixed": True}],
...
def test_07_ambiguous_input_clock():
    sv = (
        "module m(input clk_a, clk_b, d, output reg qa, qb); "
    ...
    amb = [m for m in rpt.missing_information if m["object"] == "d" and "clock" in m["category"]]
    assert amb
...
def test_08_unknown_input_clock():
    ...
    sv = "module m(input clk, d, output q); assign q = d; endmodule"
    _, _, _, cset, _, rpt = _run(
        sv, user_clocks=[{"name": "clk", "period": "10ns"}], user_inputs={"d": {"delay": "2ns"}}
    )
    assert _by_type(cset, ConstraintType.SET_INPUT_DELAY) == []
...
def test_12_user_io_delay_preserved():
    ...
    _, _, _, cset, _, _ = _run(
        sv,
        user_clocks=[{"name": "clk", "period": "10ns"}],
...
def test_15_sync_reset_candidate_not_emitted():
    ...
    assert _by_type(cset, ConstraintType.SET_FALSE_PATH) == []
    assert any(
        "sync" in (w.get("message", "").lower() + a.get("message", "").lower())
        for w in rpt.warnings
        for a in rpt.ambiguities
    ) or any("rst" in m["object"] for m in rpt.missing_information)
...
def test_17_unknown_relationship_requires_confirmation():
    ...
    _, _, _, cset, _, rpt = _run(
        sv, user_clocks=[{"name": "clk_a", "period": "10ns"}, {"name": "clk_b", "period": "8ns"}]
    )
...
def test_18_explicit_async_relationship():
    ...
    _, _, _, cset, _, _ = _run(
        ...
        user_rels=[
            ClockRelationshipSpec(
                clocks=["clk_a", "clk_b"], relationship="asynchronous", fixed=True
            )
        ],
    )
...
def test_19_explicit_sync_relationship_no_groups():
    ...
    _, _, _, cset, _, _ = _run(
        ...
        user_clocks=[{"name": "clk_a", "period": "10ns"}, {"name": "clk_b", "period": "10ns"}],
        user_rels=[
            ClockRelationshipSpec(clocks=["clk_a", "clk_b"], relationship="synchronous", fixed=True)
        ],
    )
    assert _by_type(cset, ConstraintType.SET_CLOCK_GROUPS) == []
...
def _gclk_sv():
    ...
    return (
        "module m(input clk, rst_n, d, output reg q, q2); "
...
def test_20_generated_clock_candidate_not_auto_emitted():
    ...
    assert tg.generated_clock_candidates or any(
        "generated_clock" in m["category"] for m in rpt.missing_information
    )
...
def test_22_clock_mux_candidate_not_auto_emitted():
    sv = (
        "module m(input clk_a, clk_b, sel, d, output reg q); "
        "wire c = sel ? clk_a : clk_b; "
        "always_ff @(posedge c) q<=d; endmodule"
    )
    _, _, _, cset, _, _ = _run(
        sv, user_clocks=[{"name": "clk_a", "period": "10ns"}, {"name": "clk_b", "period": "10ns"}]
    )
...
def test_30_report_deterministic():
    ...
    def run_once():
        ...
        return cset.to_canonical_json(indent=None), [
            (m["id"], m["category"], m["object"]) for m in rpt.missing_information
        ]
...
def test_adv_signal_named_clk_used_as_data():
    sv = (
        "module m(input real_clk, clk, d, output reg q); "
...
def test_adv_signal_named_reset_used_as_data():
    sv = (
        "module m(input clk, reset, d, output reg q); "
...
def test_adv_unsupported_event_control_conservative():
    ...
    sv = (
        "module m(input a, b, d, output reg q); always_ff @(posedge a or posedge b) q<=d; endmodule"
    )
    _, tg, _, cset, _, rpt = _run(sv)
