...
def _result(record):
    ...
    return {
        ...
        "blocker_categories": sorted(
            {item.category for item in assessment.issues if item.severity.value == "BLOCKER"}
        ),
        "warning_categories": sorted(
            {item.category for item in assessment.issues if item.severity.value == "WARNING"}
        ),
...
def _projections(tmp_path: Path) -> dict:
    ...
    warning_args = args | {
        "policy": replace(args["policy"], allow_release_with_warnings=True),
        "validation": _validation(status="PASS_WITH_WARNINGS"),
    }
    warning_candidate = create_release_candidate(cset, **warning_args)
    warning_released = release_constraint_set(
        warning_candidate, cset, actor=ReviewActor.from_value("alice"), **warning_args
    )
    scenarios = _ucm(scenarios=("FAST", "SLOW"))
    scenario_args = _clean_inputs(scenarios)
    selected = create_release_candidate(
        scenarios,
        scenario_ids=("FAST",),
        **(
            scenario_args
            | {"policy": replace(scenario_args["policy"], require_all_active_scenarios=False)}
        ),
    )
    ...
    conflict_scope = create_release_candidate(
        scenarios, scenario_ids=("FAST",), all_active_scenarios=True, **scenario_args
    )
    sdc = tmp_path / "known.sdc"
    ...
    required_sdc = create_release_candidate(
        cset, **(args | {"policy": sdc_policy, "sdc_path": sdc})
    )
    required_artifact_missing = create_release_candidate(
        cset, **(args | {"policy": artifact_policy})
    )
    required_artifact = create_release_candidate(
        cset, **(args | {"policy": artifact_policy, "additional_artifacts": {"trace": artifact}})
    )
    formal_policy = replace(
        args["policy"], require_formal_for_constraint_types=(ConstraintType.CREATE_CLOCK.value,)
    )
    formal_missing = create_release_candidate(cset, **(args | {"policy": formal_policy}))
    incomplete_coverage = create_release_candidate(
        cset,
        **(
            args
            | {
    ...
    changed = cset.clone()
    changed.create_clock("later", 12e-9, source="later", source_kind=SourceKind.USER)
    stale = assess_constraint_release(changed, release=clean)
    revoked = revoke_release(
        released, cset, actor=ReviewActor.from_value("bob"), comment="withdraw"
    )
    newer = _ucm()
    newer.create_clock("later", 12e-9, source="later", source_kind=SourceKind.USER)
    ...
    return {
        ...
        "02_clean_explicit_release": {
            "status": released.status.value,
            "previous": bool(released.previous_release_id),
        ...
        "08_warning_explicit_release": {
            "status": warning_released.status.value,
        ...
        "15_required_sdc_supplied": {
            "status": required_sdc.status.value,
        ...
        "17_required_artifact_supplied": {
            "status": required_artifact.status.value,
        ...
        "20_unsupported_semantics": {
            "status": unsupported_record.status.value,
            "semantic_identity": unsupported_record.snapshot.ucm_semantic_identity or "UNKNOWN",
        },
        "21_stale_ucm": {
            "current_status": stale.current_status.value,
            "reason_count": len(stale.staleness_reasons),
        },
        "22_explicit_revocation": {
            "status": revoked.status.value,
            "history": [item.kind.value for item in revoked.decision_history],
        },
        "23_explicit_supersession": {
            "status": superseding.status.value,
            "supersedes": superseding.supersedes_release_id == released.id,
        },
        "24_package_manifest": {
            "kind": package.kind,
            "artifact_kinds": sorted(item.kind for item in package.artifacts),
        },
        "25_package_verification": {
            "status": package_verification.status.value,
            "count": len(package_verification.verified_artifact_ids),
        },
        "26_package_corruption": {
            "status": corrupted_verification.status.value,
        ...
        "29_identity_binding": {
            "content": clean.identity.ucm_content_identity == clean.snapshot.ucm_content_identity,
            "semantic": clean.identity.ucm_semantic_identity
            == clean.snapshot.ucm_semantic_identity,
