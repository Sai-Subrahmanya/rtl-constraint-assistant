...
def _accepted_projection() -> dict:
    with _fixed_rtl_parse():
        design, timing, config, cset, advisory, _candidate, receipt = _accepted_context(
            knowledge=True
        )
...
def _source_classification_projection() -> dict:
    ...
    user.provenance = ProvenanceRecord(
        source_kind=SourceKind.USER, created_by="user", created_at=epoch
    )
    imported = Constraint(
        id="IMP-1",
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["imp"],
        values={"name": "imp", "period": 10e-9, "source": "imp"},
    ...
    inferred = Constraint(
        ...
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["inf"],
        values={"name": "inf", "period": 8e-9, "source": "inf"},
    ...
    knowledge = Constraint(
        ...
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["know"],
        values={"name": "know", "period": 8e-9, "source": "know"},
        source_kind=SourceKind.INFERENCE,
        provenance=ProvenanceRecord(
            source_kind=SourceKind.INFERENCE,
            created_at=epoch,
    ...
    system = Constraint(
        ...
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["rtl"],
        values={"name": "rtl", "period": 12e-9, "source": "rtl"},
        source_kind=SourceKind.RTL,
        provenance=ProvenanceRecord(
            source_kind=SourceKind.RTL, created_by="parser", created_at=epoch
        ),
    )
    unknown = Constraint(
        id="UNK-1",
        type=ConstraintType.CREATE_CLOCK,
        target_objects=["legacy"],
        values={"name": "legacy", "period": 12e-9, "source": "legacy"},
        source_kind=SourceKind.INFERENCE,
    ...
    return {
        item.constraint_id: {
            ...
            "import_file": (
                item.provenance.import_meta.source_file
                if item.provenance and item.provenance.import_meta
                else None
            ),
...
def _validation_formal_projection() -> dict:
    ...
    entry = build_constraint_lineage(
        cset, validation=validation, formal_results=(formal,)
    ).constraints[0]
    return {
...
def _mcmm_and_unknown_semantics_projection() -> dict:
    ...
    return {
        ...
        "unknown_constraint_ids": [
            item.before_constraint_id for item in changes if item.kind.value == "UNKNOWN"
        ],
    }
