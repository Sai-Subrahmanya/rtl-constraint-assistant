...
def _result(assessment):
    return {
        ...
        "warning_categories": sorted(
            {item.category for item in assessment.findings if item.severity.value == "WARNING"}
        ),
...
def _projections() -> dict:
    ...
    ready_warnings = _assessment(
        cset, policy=warnings_policy, readiness=_readiness(ReadinessStatus.READY_WITH_WARNINGS)
    )
    ...
    approved = approve_review(
        clean.review, cset, actor=ReviewActor.from_value("alice"), comment="clean"
    )
    approved_warning = approve_review_with_warnings(
        ready_warnings.review,
        cset,
    ...
    rejected = reject_review(
        clean.review, cset, actor=ReviewActor.from_value("alice"), comment="reject"
    )
    deferred = defer_review(
        clean.review, cset, actor=ReviewActor.from_value("alice"), comment="defer"
    )
    changed = cset.clone()
    changed.get(cset.clocks()[0].id).values["period"] = 8e-9
    stale = assess_constraint_review(changed, review=approved)
    scoped = _assessment(
        _ucm(scenarios=("FAST", "SLOW")),
        scenario_ids=("FAST",),
    ...
    design, timing, config, applied_ucm, advisory, _candidate, receipt = _accepted_context(
        knowledge=True
    )
    ...
    formal_policy = ReviewPolicy(
        require_formal_for_constraint_types=(ConstraintType.CREATE_CLOCK.value,)
    )
    formal_verified = _assessment(
        cset,
    ...
    coverage_incomplete = _assessment(
        cset,
        validation=_validation(
    ...
    superseding = supersede_review(
        approved, changed, readiness=_readiness(), validation=_validation()
    )
    return {
        ...
        "explicit_APPROVE": {
            ...
            "decision": approved.approval.kind.value,
        },
        "explicit_APPROVE_WITH_WARNINGS": {
            "status": approved_warning.status.value,
            "decision": approved_warning.approval.kind.value,
        },
        "explicit_REJECT": {
            "status": rejected.status.value,
            "decision": rejected.approval.kind.value,
        },
        "explicit_DEFER": {
            "status": deferred.status.value,
            "decision": deferred.approval.kind.value,
        },
        "stale_approved_review": {
            "current_status": stale.current_status.value,
            "reasons": list(stale.staleness_reasons),
        },
        ...
        "inference_application_ucm_lineage_review": {
            ...
            "lineage_reference": chained.review.reviewed_snapshot.lineage_snapshot_identity
            == lineage.snapshot.snapshot_identity,
        },
        "unsupported_unknown_lineage": {
            "semantic_identity": unsupported_lineage.review.reviewed_snapshot.ucm_semantic_identity
            or "UNKNOWN"
        },
        "formal_UNVERIFIED": _result(formal_unverified),
        "supplied_formal_VERIFIED": _result(formal_verified),
        "validation_warning_vs_blocker": {
            "warning_status": _result(
                _assessment(cset, validation=_validation(status="PASS_WITH_WARNINGS"))
            )["approval_possible"],
            "blocker_status": _result(_assessment(cset, validation=_validation(status="FAIL")))[
                "approval_possible"
            ],
        },
