...
def rule_gclk_001_divider_candidate(
    design: Design, tg: TimingGraph, *, _run_ts: str | None = None, **kw
) -> InferenceResult:
    rid = "GCLK-001"
    res = InferenceResult(
        rule_id=rid,
        rule_name="clock_divider_candidate",
    ...
    for gc in tg.generated_clock_candidates:
        ...
        res.add_missing(
            MissingInformation(
                ...
                suggested_inputs=[
                    {"field": "intent"},
                    {"field": "master_clock", "value": master},
                    {"field": "divide_by"},
                ],
                blocking=False,
                rule_id=rid,
...
def rule_gclk_002_gated_clock_candidate(
    design: Design, tg: TimingGraph, *, _run_ts: str | None = None, **kw
) -> InferenceResult:
    rid = "GCLK-002"
    res = InferenceResult(
        rule_id=rid,
        rule_name="gated_clock_candidate",
        confidence=Confidence.LOW,
        result_status=InferenceResultStatus.NO_FINDING,
    )
    for cg in tg.clock_gating_candidates:
        ...
        res.add_missing(
            MissingInformation(
                ...
                category="gated_clock_intent",
                object=out,
                severity="WARNING",
                requirement_level=RequirementLevel.UNSAFE_TO_INFER,
                message=f"Clock-gating intent confirmation required for '{out}'",
                rationale=(
                    "A combinational gate between clock and register clock pins was found; "
...
def rule_gclk_003_mux_candidate(
    design: Design, tg: TimingGraph, *, _run_ts: str | None = None, **kw
) -> InferenceResult:
    rid = "GCLK-003"
    res = InferenceResult(
        rule_id=rid,
        rule_name="clock_mux_candidate",
        confidence=Confidence.LOW,
        result_status=InferenceResultStatus.NO_FINDING,
    )
    for cm in tg.clock_mux_candidates:
        ...
        res.add_missing(
            MissingInformation(
                ...
                category="clock_mux_intent",
                object=out,
                severity="WARNING",
                requirement_level=RequirementLevel.UNSAFE_TO_INFER,
                message=f"Clock-mux exclusivity/mode confirmation required for '{out}'",
                rationale=(
                    "Multiple clock sources fan into a mux-like structure feeding a clock "
