...
def create_release_candidate(
    ...
    *,
    ...
    snapshot = _snapshot(
        cset, config, design, timing_graph, review, readiness, validation, lineage, formal_results
    )
    ...
    root_id = _release_root_id(
        snapshot, scope, policy, evidence, dependencies, supersedes_release_id
    )
    ...
    baseline = ReleaseBaseline(
        id="RBL-"
        + stable_hash(
            {"release_root_id": root_id, "snapshot": snapshot.to_dict(), "scope": scope.to_dict()}
        )[:20],
        snapshot=snapshot,
        scope=scope,
        review_id=snapshot.review_id or None,
    )
    ...
    return ConstraintRelease(
        ...
        external_eda_signoff=(
            ExternalEDASignoffStatus.EVIDENCE_SUPPLIED
            if external_refs
    ...
    *,
    ...
    if release is None:
        ...
        raise ReleaseDecisionError(
            "A stored release must be assessed under its recorded ReleasePolicy."
        )
    ...
    scope = _scope(
        cset,
        config,
        release.scope.requested_scenario_ids,
    ...
    use_recorded = (
        ...
        and sdc_path is None
        and not additional_artifacts
    )
    if use_recorded:
    ...
    release_with_warnings_possible = (
        ...
        and not blockers
        and release.policy.allow_release_with_warnings
    )
...
def release_constraint_set(
    ...
    *,
    ...
    decision = _decision(
        release.release_root_id,
        ReleaseDecisionKind.RELEASE,
        actor,
        comment,
        _evidence_ids(assessment.evidence),
        recorded_at,
    )
    return replace(
        release,
...
def supersede_release(
    release: ConstraintRelease, cset: ConstraintSet, **kwargs: Any
) -> ConstraintRelease:
...
def create_release_package(
    ...
    *,
    ...
    if (
        assessment.current_status in {ReleaseStatus.STALE, ReleaseStatus.REVOKED}
        or not assessment.snapshot_current
    ):
    ...
    if assessment.current_snapshot.to_dict() != release.snapshot.to_dict():
        raise ReleasePackageError(
            "Supplied package inputs do not match the released snapshot identities."
        )

    root = Path(output_dir)
    if root.exists() and any(root.iterdir()):
        raise ReleasePackageError(
            "Release package directory must be new or empty; stale contents are not repaired."
        )
    ...
    if formal_results:
        write_json(
            "formal", "formal.json", [_as_dict(item) for item in formal_results], required=False
        )
    if sdc_path is not None:
        ...
        written.append(
            _artifact("sdc", relative, manager.path(relative), required=release.policy.require_sdc)
        )
    for kind, raw_path in sorted(
        (additional_artifacts or {}).items(), key=lambda item: str(item[0])
    ):
        source = Path(raw_path)
        if not source.is_file():
            raise ReleasePackageError(
                f"Explicit supplied artifact {kind!r} is not a readable file."
            )
...
def verify_release_package(package_dir: str | Path) -> ReleaseVerification:
    ...
    try:
        ...
        issue = _issue(
            ReleaseIssueSeverity.BLOCKER, "PACKAGE", f"Cannot read package manifest: {exc}"
        )
        return ReleaseVerification(
            package_id="", status=PackageVerificationStatus.INVALID, issues=(issue,)
        )
    if package.kind != "rca_constraint_release_package" or not package.id:
        issues.append(
            _issue(
                ReleaseIssueSeverity.BLOCKER,
                "PACKAGE",
                "Package manifest kind or identity is invalid.",
            )
        )
    expected_package_id = _package_id_values(
        package.release_id,
    ...
    if package.id != expected_package_id:
        issues.append(
            _issue(
                ReleaseIssueSeverity.BLOCKER,
                "PACKAGE",
                "Package identity does not match manifest contents.",
            )
        )
            ...
            if artifact.required:
                issues.append(
                    _issue(
                        ReleaseIssueSeverity.BLOCKER,
                        "ARTIFACT",
                        f"Required package artifact is missing: {artifact.relative_path}",
                        (artifact.id,),
                    )
                )
            continue
        ...
        if actual_hash != artifact.sha256 or actual_size != artifact.size_bytes:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "ARTIFACT",
                    f"Artifact hash or size mismatch: {artifact.relative_path}",
                    (artifact.id,),
                )
            )
            continue
    ...
    if release is not None:
        if release.id != package.release_id or release.identity.id != package.release_identity:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Package release identity does not match release record.",
                )
            )
        expected_root_id = _release_root_id(
            release.snapshot,
            release.scope,
            release.policy,
            release.evidence,
        ...
        expected_identity = _release_identity(
            release.snapshot, release.scope, release.policy, release.evidence, release.dependencies
        )
        if release.release_root_id != expected_root_id or release.identity.id != expected_identity:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Release record identity is not bound to its evidence baseline.",
                )
            )
        if (
            release.identity.ucm_content_identity != release.snapshot.ucm_content_identity
            ...
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Release identity summary differs from its snapshot or exact scope.",
                )
            )
        if (
            release.snapshot.to_dict() != package.snapshot.to_dict()
            or release.scope.to_dict() != package.scope.to_dict()
        ):
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Release record snapshot or scope differs from package manifest.",
                )
            )
        if (
            tuple(sorted(release.dependencies, key=lambda item: (item.kind, item.identity)))
            != package.dependencies
        ):
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Release dependencies differ from package manifest.",
                )
            )
        if release.status == ReleaseStatus.REVOKED:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Package contains an explicitly revoked release record.",
                )
            )
            ...
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "RELEASE",
                    "Package release record is not explicitly released.",
                )
            )
    if cset is not None:
        if stable_hash_cset(cset) != package.snapshot.ucm_content_identity:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "UCM",
                    "Packaged UCM content identity differs from manifest.",
                )
            )
        if _semantic_ucm_identity(cset) != package.snapshot.ucm_semantic_identity:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "UCM",
                    "Packaged UCM semantic identity differs from manifest.",
                )
            )
        if tuple(sorted(cset.constraints)) != package.snapshot.constraint_ids:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "UCM",
                    "Packaged UCM constraint IDs differ from manifest.",
                )
            )
        if tuple(sorted(cset.scenarios)) != package.snapshot.scenario_ids:
            issues.append(
                _issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "MCMM_SCOPE",
                    "Packaged UCM scenario IDs differ from manifest.",
                )
            )
...
class ConstraintReleaseEngine:
    ...
    def release(
        self, release: ConstraintRelease, cset: ConstraintSet, **kwargs: Any
    ) -> ConstraintRelease:
...
def _scope(
    cset: ConstraintSet, config: Any | None, scenario_ids: Iterable[str], all_active: bool
) -> ReleaseScope:
    if config is not None:
        ...
        active = tuple(
            sorted(
                sid
                for sid, scenario in cset.scenarios.items()
    ...
    return ReleaseScope(
        ...
        released_scenario_ids=released,
...
def _snapshot(
    ...
    readiness: ConstraintReadinessReport | None,
    validation: ValidationResult | None,
    lineage: ConstraintLineageReport | None,
    formal_results: Iterable[Any],
) -> ReleaseSnapshot:
    ...
    return ReleaseSnapshot(
        ...
        review_snapshot_identity=(
            review_record.reviewed_snapshot.ucm_content_identity
        ...
        formal_identity=_identity([_as_dict(item) for item in formal_results])
        if formal_results
        else "",
...
def _evidence_and_gates(
    ...
    lineage: ConstraintLineageReport | None,
    formal_results: Iterable[Any],
    sdc_path: str | Path | None,
    additional_artifacts: Mapping[str, str | Path] | None,
    ...
    def add_evidence(
        ...
        *,
        ...
        item = _evidence(
            authority, reference, status, required=required, stale=stale, details=details
        )
    ...
    ucm_ref = add_evidence(
        "CANONICAL_UCM",
        snapshot.ucm_content_identity,
        "CURRENT",
        required=True,
        details={
            "constraint_ids": list(snapshot.constraint_ids),
            "scenario_ids": list(snapshot.scenario_ids),
    ...
    if not snapshot.ucm_semantic_identity:
        issue(
            ...
            "UCM_SEMANTICS",
            "UCM has unsupported semantics; exact content is bound but semantic equivalence is unknown.",
            (ucm_ref.id,),
        )

    scope_ref = add_evidence(
        "MCMM_SCOPE",
        scope.scenario_definition_identity,
        "CURRENT",
        required=True,
    ...
    if scope.unknown_scenario_ids:
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "MCMM_SCOPE",
            "Requested release scope contains unknown or conflicting scenario selection.",
            (scope_ref.id,),
        )
    elif policy.require_all_active_scenarios and not set(scope.active_scenario_ids).issubset(
        scope.released_scenario_ids
    ):
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "MCMM_SCOPE",
            "ReleasePolicy requires all active MCMM scenarios to be released.",
            (scope_ref.id,),
        )
    elif scope.scope_kind == "SELECTED_SCENARIOS":
        issue(
            ReleaseIssueSeverity.INFORMATION,
            "MCMM_SCOPE",
            "Release is explicitly limited to selected active scenarios.",
            (scope_ref.id,),
        )
    ...
    return (
        tuple(sorted(evidence, key=lambda item: item.id)),
...
def _context_gate(snapshot, policy, add, dependencies, issue) -> None:
    ...
    for category, identity, required, detail in (
        (
            ...
            policy.require_configuration,
            "Supplied project/configuration identity.",
        ),
        (
            "DESIGN",
            snapshot.design_identity,
            policy.require_design,
            "Supplied design-model identity.",
        ),
        (
            "TIMING_GRAPH",
            snapshot.timing_graph_identity,
            policy.require_timing_graph,
            "Supplied timing-graph identity.",
        ),
    ):
        ref = add(
            category,
            identity or "UNSUPPLIED",
            "CURRENT" if identity else "MISSING",
            required=required,
        )
        if identity:
...
def _review_gate(cset, snapshot, scope, policy, review, add, dependencies, issue) -> None:
    if review is None:
        ...
        if policy.require_review:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "REVIEW",
                "No explicit approved Step-31 review was supplied.",
                (ref.id,),
            )
        return
    ...
    ref = add(
        ...
        details={
            "reviewed_ucm_identity": record.reviewed_snapshot.ucm_content_identity,
            "review_status_at_record": record.status.value,
        },
    )
    dependencies.append(
        ReleaseDependency(
            "review", record.id, policy.require_review, "Explicit Step-31 review record."
        )
    )
    if record.reviewed_snapshot.ucm_content_identity != snapshot.ucm_content_identity:
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "REVIEW",
            "Review is bound to a different canonical UCM snapshot.",
            (ref.id,),
        )
        ...
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "REVIEW",
            "Review MCMM scope does not cover the exact release scenario scope.",
            (ref.id,),
        )
    elif (
        record.scope.scenario_definition_identity
        and scope.scenario_definition_identity
        and record.scope.scenario_definition_identity != scope.scenario_definition_identity
    ):
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "REVIEW",
            "Review MCMM scenario definition differs from release scope.",
            (ref.id,),
        )
        ...
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "REVIEW",
            "Review is stale against current supplied evidence.",
            (ref.id,),
        )
        ...
        if policy.allow_review_with_warnings:
            issue(
                ReleaseIssueSeverity.WARNING,
                "REVIEW",
                "Review was approved with warnings.",
                (ref.id,),
            )
        else:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "REVIEW",
                "ReleasePolicy does not permit a review approved with warnings.",
                (ref.id,),
            )
    else:
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "REVIEW",
            f"Review status {status} is not an explicit approval.",
            (ref.id,),
        )
...
def _readiness_gate(cset, snapshot, policy, readiness, add, dependencies, issue) -> None:
    if readiness is None:
        ...
        if policy.require_readiness:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "READINESS",
                "No existing Step-29 readiness report was supplied.",
                (ref.id,),
            )
        return
    status = readiness.status.value
    readiness_ucm = getattr(readiness, "source_snapshot_identity", {}).get("constraint_set")
    stale = bool(readiness.stale_evidence) or bool(
        readiness_ucm and readiness_ucm != snapshot.ucm_content_identity
    )
    ref = add(
        "READINESS",
        _identity(readiness.to_dict()),
        status,
        required=policy.require_readiness,
        stale=stale,
        details={
    ...
    if stale:
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "READINESS",
            "Readiness evidence is stale or references another UCM.",
            (ref.id,),
        )
        ...
        _unresolved(
            policy,
            "READINESS",
            f"Readiness status {status} is not allowed by ReleasePolicy.",
            ref.id,
            issue,
        )
    if readiness.blockers:
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "READINESS",
            "Readiness evidence contains unresolved blockers.",
            (ref.id,),
        )
    if readiness.warnings:
        issue(
            ReleaseIssueSeverity.WARNING,
            "READINESS",
            "Readiness evidence contains warnings.",
            (ref.id,),
        )
...
def _validation_gate(policy, validation, add, dependencies, issue) -> None:
    if validation is None:
        ref = add(
            "VALIDATION", "UNSUPPLIED", "MISSING", required=policy.require_validation_evidence
        )
        if policy.require_validation_evidence:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "VALIDATION",
                "No existing validation result was supplied.",
                (ref.id,),
            )
        if policy.require_complete_coverage:
            ...
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "COVERAGE",
                "No existing coverage result was supplied.",
                (cref.id,),
            )
        return
    ...
    ref = add(
        "VALIDATION",
        _identity(validation.as_dict()),
        status,
        required=policy.require_validation_evidence,
    ...
    dependencies.append(
        ReleaseDependency(
            "validation",
            ref.reference_id,
            policy.require_validation_evidence,
    ...
    if normalized in {"PASS", "READY", "VALID", "COMPLETE"}:
        ...
        issue(
            ReleaseIssueSeverity.WARNING,
            "VALIDATION",
            "Validation result contains warnings.",
            (ref.id,),
        )
        ...
        _unresolved(
            policy,
            "VALIDATION",
            f"Validation status {status} is not release-complete.",
            ref.id,
            issue,
        )
    for raw in getattr(validation.report, "issues", ()):
        ...
        blocking = bool(getattr(raw, "blocking", False)) or severity in {
            "CRITICAL",
            "ERROR",
            "HIGH",
        }
        issue_ref = add(
            "VALIDATION_ISSUE", issue_id, severity or "UNKNOWN", details={"issue": _as_dict(raw)}
        )
        if blocking:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "VALIDATION",
                f"Validation reports blocking issue {issue_id}.",
                (ref.id, issue_ref.id),
            )
            ...
            issue(
                ReleaseIssueSeverity.WARNING,
                "VALIDATION",
                f"Validation reports warning issue {issue_id}.",
                (ref.id, issue_ref.id),
            )
    coverage = getattr(validation, "coverage", None)
    if coverage is None:
        ...
        if policy.require_complete_coverage:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "COVERAGE",
                "Validation result has no coverage evidence.",
                (cref.id,),
            )
        return
    ...
    cref = add(
        ...
        required=policy.require_complete_coverage,
        details={"coverage": data},
    )
    dependencies.append(
        ReleaseDependency(
            "coverage",
    ...
    if not complete:
        severity = (
            ReleaseIssueSeverity.BLOCKER
            if policy.require_complete_coverage
            else ReleaseIssueSeverity.WARNING
        )
    ...
    if lineage is None:
        ...
        if policy.require_lineage:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "LINEAGE",
                "No Step-30 lineage report was supplied.",
                (ref.id,),
            )
        return
    matches = lineage.snapshot.snapshot_identity == snapshot.ucm_content_identity
    stale = not matches or any(
        event.stale
        for entry in lineage.constraints
        for event in (*entry.events, *entry.validation_events, *entry.formal_events)
    )
    ref = add(
        "LINEAGE",
        _identity(lineage.to_dict()),
        "CURRENT" if matches else "STALE",
        required=policy.require_lineage,
        stale=stale,
        details={
            "lineage_snapshot_identity": lineage.snapshot.snapshot_identity,
            "constraint_ids": sorted(item.constraint_id for item in lineage.constraints),
        },
    )
    dependencies.append(
        ReleaseDependency(
            "lineage", ref.reference_id, policy.require_lineage, "Existing Step-30 lineage report."
        )
    )
    if not matches:
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "LINEAGE",
            "Lineage report is bound to another UCM snapshot.",
            (ref.id,),
        )
    elif stale:
        issue(
            ReleaseIssueSeverity.WARNING,
            "LINEAGE",
            "Lineage retains stale source evidence.",
            (ref.id,),
        )
...
def _formal_gate(cset, policy, formal_results, add, dependencies, issue) -> None:
    ...
    for raw in formal_results:
        ...
        dependencies.append(
            ReleaseDependency(
                "formal", _identity(data), False, "Supplied existing formal evidence."
            )
        )
        if constraint_id:
        ...
        if status.upper() in {"FAILED", "INVALID", "ERROR"}:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "FORMAL",
                f"Formal evidence for {reference} reports {status}.",
                (ref.id,),
            )
            ...
            issue(
                ReleaseIssueSeverity.WARNING,
                "FORMAL",
                f"Formal evidence for {reference} remains {status}.",
                (ref.id,),
            )
...
def _artifact_gate(policy, sdc_path, additional, add, dependencies, issue) -> None:
    ...
    if sdc_path is None:
        ...
        if policy.require_sdc:
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "SDC",
                "ReleasePolicy requires an explicitly supplied SDC artifact.",
                (ref.id,),
            )
    else:
        path = Path(sdc_path)
        if not path.is_file():
            ...
            issue(
                ReleaseIssueSeverity.BLOCKER,
                "SDC",
                "Explicit supplied SDC artifact is not readable.",
                (ref.id,),
            )
        else:
    ...
    for kind, raw in sorted((additional or {}).items(), key=lambda item: str(item[0])):
        ...
        if not path.is_file():
            ref = add(
                ...
                "UNREADABLE",
                "INVALID",
                required=label in policy.required_artifact_kinds,
            )
            if label in policy.required_artifact_kinds:
                issue(
                    ReleaseIssueSeverity.BLOCKER,
                    "ARTIFACT",
                    f"Required supplied artifact {label!r} is not readable.",
                    (ref.id,),
                )
            continue
        digest = hash_file(path)
        ...
        dependencies.append(
            ReleaseDependency(
                f"artifact:{label}",
                digest,
                label in policy.required_artifact_kinds,
    ...
    for required in set(policy.required_artifact_kinds) - seen:
        ...
        issue(
            ReleaseIssueSeverity.BLOCKER,
            "ARTIFACT",
            f"ReleasePolicy requires artifact kind {required!r}.",
            (ref.id,),
        )


def _candidate_status(
    scope: ReleaseScope, snapshot: ReleaseSnapshot, issues: Iterable[ReleaseIssue]
) -> ReleaseStatus:
    if scope.unknown_scenario_ids:
...
def _staleness_reasons(
    ...
    if prior.ucm_semantic_identity != current.ucm_semantic_identity:
        reasons.append(
            "Canonical UCM semantic identity differs from release baseline or is unsupported."
        )
...
def _read_release_record(
    root: Path, package: ReleasePackage, issues: list[ReleaseIssue]
) -> ConstraintRelease | None:
    ...
    try:
        ...
        issues.append(
            _issue(
                ReleaseIssueSeverity.BLOCKER,
                "RELEASE",
                f"Cannot read packaged release record: {exc}",
            )
        )
        return None


def _read_package_ucm(
    root: Path, package: ReleasePackage, issues: list[ReleaseIssue]
) -> ConstraintSet | None:
    ...
    try:
        ...
        issues.append(
            _issue(
                ReleaseIssueSeverity.BLOCKER, "UCM", f"Cannot read packaged canonical UCM: {exc}"
            )
        )
        return None
...
def _package_id_values(
    release_id: str,
    release_identity: str,
    snapshot: ReleaseSnapshot,
    scope: ReleaseScope,
    dependencies: Iterable[ReleaseDependency],
        ...
        + stable_hash(
            {
                ...
                "dependencies": [
                    item.to_dict()
                    for item in sorted(dependencies, key=lambda item: (item.kind, item.identity))
                ],
                "artifacts": [
                    item.to_dict()
...
def _decision(
    ...
    return ReleaseDecision(
        id="RDC-" + stable_hash(identity)[:20],
        kind=kind,
        actor=actor,
        comment=comment,
        release_root_id=root_id,
...
def _decision_release_id(release: ConstraintRelease, decision: ReleaseDecision) -> str:
    return (
        ...
        + stable_hash(
            {
                "release_root_id": release.release_root_id,
                "previous_release_id": release.id,
                "decision_id": decision.id,
                "history": [item.id for item in release.decision_history] + [decision.id],
            }
        )[:20]
    )
...
def _unresolved(
    policy: ReleasePolicy, category: str, message: str, evidence_id: str, issue
) -> None:
    severity = (
        ReleaseIssueSeverity.WARNING
        if category in set(policy.allowed_unresolved_evidence_categories)
...
def _semantic_ucm_identity(cset: ConstraintSet) -> str:
    ...
    return stable_hash(
        [
            {"id": item.id, "semantic": normalize_constraint(item)}
...
def _validation_issue_ids(validation: ValidationResult) -> list[str]:
    return sorted(
        str(getattr(item, "issue_id", "") or _identity(_as_dict(item)))
...
def _release_failure(assessment: ReleaseAssessment) -> str:
    messages = [
        item.message for item in assessment.issues if item.severity == ReleaseIssueSeverity.BLOCKER
    ]
    if not messages:
...
def _sorted_issues(values: Iterable[ReleaseIssue]) -> list[ReleaseIssue]:
    order = {
        ReleaseIssueSeverity.BLOCKER: 0,
        ReleaseIssueSeverity.WARNING: 1,
        ReleaseIssueSeverity.INFORMATION: 2,
    }
    return sorted(
        values,
        key=lambda item: (order[item.severity], item.category, item.scenario_id or "", item.id),
    )
